...
class FileService:
    ...
    async def _process_single_file(self, file: UploadFile, job_id: str, user_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            existing = await db.supabase.table("documents").select("id").eq("content_hash", content_hash).execute()
            if existing.data:
                return {
                    "success": False,
