...
from app.models.enums import FileStatus, BatchStatus
...
class ProcessingService:
    ...
    async def process_batch(self, batch_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            files_result = await db.supabase.table("processing_files").select("id, status").eq("batch_id", batch_id).execute()
            ...
            file_ids = [f["id"] for f in files_result.data if f["status"] == FileStatus.UPLOADED.value]
            ...
            await self._update_batch_status(batch_id, final_status, processed_files=successful, failed_files=failed)
            
            logger.info(f"Batch {batch_id} processing complete: {successful} success, {failed} failed")
    ...
    async def _process_file_with_semaphore(self, semaphore: asyncio.Semaphore, file_id: str) -> Dict[str, Any]:
    ...
    async def _process_file_pipeline(self, file_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not extraction_result["success"]:
                logger.error(f"Text extraction failed for file {file_id}: {extraction_result['error']}")
                return extraction_result
            
            # Step 2: AI Metadata Extraction
            metadata_result = await self.ai_service.extract_metadata(file_id)
            if not metadata_result["success"]:
                logger.error(f"Metadata extraction failed for file {file_id}: {metadata_result['error']}")
                return metadata_result
            
            # Step 3: Embedding Generation
            embedding_result = await self.embedding_service.generate_embeddings(file_id)
            if not embedding_result["success"]:
                logger.error(f"Embedding generation failed for file {file_id}: {embedding_result['error']}")
            ...
            await self._update_file_status(file_id, FileStatus.PROCESSING_FAILED, error_message=str(e))
            return {
                "success": False,
                "file_id": file_id,
                "error": str(e)
    ...
    async def approve_file_for_library(self, file_id: str, reviewer_id: str, review_notes: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            file_result = await db.supabase.table("processing_files").select("*").eq("id", file_id).execute()
            ...
            if file_record["status"] != FileStatus.READY_FOR_REVIEW.value:
                raise ValueError(f"File {file_id} is not ready for review (status: {file_record['status']})")
            ...
            await db.supabase.table("document_chunks").update({
                "document_id": document_id
            }).eq("processing_file_id", file_id).execute()
    ...
    async def reject_file(self, file_id: str, reviewer_id: str, rejection_reason: str) -> Dict[str, Any]:
        """
    ...
    async def get_processing_status(self, batch_id: str) -> Dict[str, Any]:
        ...
        try:
            # Get batch info
            batch_result = await db.supabase.table("processing_jobs").select("*").eq("id", batch_id).execute()
            ...
            # Get file statuses
            files_result = await db.supabase.table("processing_files").select(
                "id, original_filename, status, error_message, created_at, updated_at"
    ...
    async def _update_batch_status(self, batch_id: str, status: BatchStatus, **kwargs):
        ...
        try:
            ...
            await db.supabase.table("processing_jobs").update(update_data).eq("id", batch_id).execute()
