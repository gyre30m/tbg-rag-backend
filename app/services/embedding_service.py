...
class EmbeddingService:
    ...
    async def generate_embeddings(self, file_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(chunks) > self.max_chunks_per_document:
                logger.warning(f"File {file_id} has {len(chunks)} chunks, truncating to {self.max_chunks_per_document}")
            ...
            if embeddings_result["success"]:
                ...
                logger.info(f"Successfully generated embeddings for file {file_id} ({len(chunks)} chunks)")
                return {
                    "success": True,
                    "file_id": file_id,
                    "chunk_count": len(chunks),
                    "embedding_dimension": len(embeddings_result["embeddings"][0]) if embeddings_result["embeddings"] else 0
                }
            else:
        ...
        try:
            ...
            for i in range(0, len(chunks), batch_size):
                ...
                try:
                    ...
                    return {
                        "success": False,
                        "error": f"OpenAI embedding API failed: {str(e)}"
                    }
    ...
    async def _save_embeddings(self, file_id: str, chunks: List[str], embeddings: List[List[float]]):
