...
async def list_files_pending_review(
    ...
    """
    try:
        result = await db.supabase.table("processing_files").select(
        ...
        ).eq("status", "ready_for_review").order("created_at", desc=True).range(offset, offset + limit - 1).execute()
...
async def get_processing_file_details(
    ...
    try:
        ...
        if file_data["status"] in ["embeddings_generated", "ready_for_review", "approved_for_library"]:
...
async def get_file_chunks(
    ...
    """
    try:
        result = await db.supabase.table("document_chunks").select(
            "id, chunk_index, text_content, token_count, created_at"
        ).eq("processing_file_id", file_id).order("chunk_index").range(offset, offset + limit - 1).execute()
...
async def retry_file_processing(
    ...
    try:
        ...
        result = await db.supabase.table("processing_files").select("status, retry_count").eq("id", file_id).execute()
        if not result.data:
            raise HTTPException(status_code=404, detail="Processing file not found")
        ...
        if current_status not in ["extraction_failed", "ai_failed", "embedding_failed", "processing_failed"]:
            raise HTTPException(status_code=400, detail=f"Cannot retry file with status: {current_status}")
...
async def get_processing_stats(
    ...
    try:
        ...
        recent_files_result = await db.supabase.table("processing_files").select(
            "id", count="exact"
        ).gte("created_at", yesterday).execute()
        
        recent_batches_result = await db.supabase.table("processing_jobs").select(
            "id", count="exact"
        ).gte("created_at", yesterday).execute()
